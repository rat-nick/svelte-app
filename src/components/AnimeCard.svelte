...
>
	{#if item !== null && ready}
		<div in:fade={{ duration: 1500 }}>
			...
			<div
				class="absolute top-0 left-0 right-0 h-full bg-gradient-to-b from-transparent to-black opacity-0 hover:opacity-100 transition-all flex flex-col justify-end duration-400"
			>
				<header class="card-header flex text-xl top-0 absolute">
					{item.title}
				</header>
	...
	{:else}
		<div
			class="flex placeholder w-full h-full rounded-none animate-pulse justify-center align-items-middle duration-100 fade"
		></div>
	{/if}
</div>
